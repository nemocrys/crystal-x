...
def read_from_msh(filename: str, cell_data=False, facet_data=False, gdim=None):
    ...
    if MPI.COMM_WORLD.rank == 0:
        ...
        gmsh.merge(filename)
    output = gmsh_model_to_mesh(
        gmsh.model, cell_data=cell_data, facet_data=facet_data, gdim=gdim
    )
    if MPI.COMM_WORLD.rank == 0:
...
def gmsh_model_to_mesh(model, cell_data=False, facet_data=False, gdim=None):
    ...
    if MPI.COMM_WORLD.rank == 0:
        ...
        cells, x = (
            numpy.empty([0, num_nodes], dtype=numpy.int32),
            numpy.empty([0, gdim]),
        )
    ...
    if facet_data:
        ...
        adj = AdjacencyList_int32(local_entities)
        ft = create_meshtags(
            mesh, mesh.topology.dim - 1, adj, numpy.int32(local_values)
        )
        ft.name = "Facet tags"
